...
| [TS-028](TS-028-payment-gateway-no-timeout.md) | 결제 PG 호출에 타임아웃이 없었다 — KOPIS는 걸어 두고 결제만 안 걸어, 응답 없는 PG가 Hikari 커넥션(파드당 5)까지 묶을 수 있었다 | S05 | 가용성(외부 호출 경계) | 해결(타임아웃 + 하네스 규칙 ⑱, fixture가 규칙 결함을 먼저 잡음) |
