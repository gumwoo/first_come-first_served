...
- **PG 취소 요청에 멱등키를 보내지 않는다.** 같은 결제에 취소가 두 번 나가는 경우의 수렴은
  여전히 PG 정책에 의존한다. 이건 환불 정산(PG ↔ DB 불일치 복구)과 함께 처리할 문제라
  이 변경에 넣지 않았다.
- **환불 정산은 없다.** PG 환불이 성공한 뒤 같은 트랜잭션의 DB 쓰기가 실패하면 주문은 PAID로
  롤백되는데, `PaymentReconciliationService`의 후보는 PENDING·EXPIRED뿐이라 이 상태는
  잡히지 않는다. `PaymentGateway.Inquiry`가 "승인 없음"과 "이미 취소됨"과 "조회 실패"를
  한 값(`none()`)으로 뭉개고 있어, 정산을 붙이려면 조회 계약부터 넓혀야 한다.
