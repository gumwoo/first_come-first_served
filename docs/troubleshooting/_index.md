...
| [TS-029](TS-029-ci-concurrency-stale-deploy.md) | 오래된 커밋이 최신 배포를 덮을 수 있었다 — concurrency는 동시 실행만 막고 순서는 보장하지 않아, 조상 관계를 직접 보는 4-상태 가드가 필요했다 | 전반(CI/CD) | 배포 정확성 | 해결(CI/배포 concurrency 분리 + bump-tag 조상 가드, 4상태 git 시뮬레이션 검증) |
