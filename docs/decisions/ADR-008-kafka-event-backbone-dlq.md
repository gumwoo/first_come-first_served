...
- ~~**한계 2 — 알림 유실 가능**: 브로커 다운 시 `order.paid` 알림은 유실(설계상 감수).~~
  → **S08에서 해소([[ADR-010]])**: `order.paid`는 결제와 같은 트랜잭션에 아웃박스로 적재되고 릴레이가
  재시도 발행하므로 브로커 다운·발행 전 크래시에도 유실되지 않는다(AFTER_COMMIT 브리지는 은퇴).
  다른 이벤트(좌석·큐 SSE)는 여전히 best-effort — DB가 진실원이라 재조회로 복구 가능.
