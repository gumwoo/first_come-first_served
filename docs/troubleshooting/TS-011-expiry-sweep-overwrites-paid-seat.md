...
- ③ 보상은 **finalizePaid 내 best-effort void**다. "PG 승인 성공 직후·refund 호출 전 프로세스 크래시"
  구간은 이 즉시 보상으로 닫히지 않는다 — 트랜잭션이 롤백돼 **DB에 흔적 자체가 없기** 때문에 아웃박스
  (ADR-010)로도 못 닫는다. → **S08 2단계 정산 잡([[ADR-011]])이 안전망**으로 닫는다: 주문 기준 후보를
  뽑아 PG에 조회하고, "우리는 미확정인데 PG엔 승인"이면 취소(void). 탐지·보상은 정산 주기만큼 지연된다.
  가상계좌(vbank) 경로의 보상 취소 의미(입금 반환)는 실 PG 배선 시 별도 검토.
