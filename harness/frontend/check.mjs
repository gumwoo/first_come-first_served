...
// 브라우저는 같은 오리진으로 요청하고 Next가 백엔드로 프록시한다(next.config.mjs).
// OAuth처럼 전체 이동으로 도달하는 경로도 rewrites에 있어야 한다. 콜백(/login/oauth2/code/*)이
// 빠지면 배포에서만 404가 난다(TS-017. 로컬은 Next와 Spring이 같은 머신이라 드러나지 않는다).
