...
## 한계 / 남은 것(별건, 후속)
- 계약(`contracts/events.yaml`)은 `seat.held`/`seat.hold.released`/`seat.hold.expired` 3종을
  선언하나 구현은 `seat.hold.expired`만 broadcast → 계약 갭(경미).
- `docs/common/api-contract.md`는 `seat.hold.expired`를 `/sse/orders/:id` 채널로 적어 stale
  (실제·`docs/rules/domain/seat.md`는 `/sse/events/{id}/seats`). 문서 정합 후속.
