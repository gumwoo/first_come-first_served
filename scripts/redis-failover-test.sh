...
#
# ADR-012가 미리 적어 둔 대로 Redis 복제는 비동기다. 최근 쓰기가 유실될 수 있고,
# 그때 "요청은 200인데 대기열에서 사라진" 상태가 된다. 그래서 5xx만 세지 않고
# status가 유지되는지를 따로 센다(k6의 position_missing_total).
#
# 사용:
...
RATE=25
# 대기열 admit-ttl(300초)보다 짧게 잡는다. 길면 토큰이 자연 만료돼 페일오버와 무관한
...
cleanup() {
  ...
  # Job을 지우기 전에 로그를 회수한다. 뒤 단계가 실패해 trap이 Job을 지워도
  # 주 측정 로그는 남아야 한다.
...
for i in $(seq 1 30); do
  # `2>/dev/null || true`로 감싸면 CLI 실패가 REC=""이 되어 "재동기화 없음"으로 오판한다.
