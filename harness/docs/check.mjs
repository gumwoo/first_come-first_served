...
//
// 백엔드 규칙 9)·15)는 "코드 → 문서" 방향(코드가 가리키는 문서가 있는가)만 본다.
// 이 하네스는 반대 방향, 문서가 코드 상태를 단언하는 경우를 본다
// (예: "아직 없다"고 적힌 리소스가 실존, "(미수정)"으로 가리킨 TS가 이미 해결).
//
// 자연어 자기모순("했다" vs "안 했다")은 다루지 않는다. 기계가 확인할 수 있는 단언만 검사한다.
...
// ---------- 16) "아직 없다"고 단언한 경로가 실제로 존재하면 실패 ----------
//
// 자연어로만 "없다"고 쓰면 기계가 못 읽는다. 그래서 경로를 백틱으로 적는 것을 규약으로 삼고,
...
//
// 인접성을 요구하는 이유: "같은 줄에 참조가 있고 어딘가에 (미수정)이 있으면"으로
// 판정하면 상태를 설명하는 문장 자체를 잡는다.
//   TS-902는 이미 해결됐으므로 위 "(미수정)"은 거짓이다.   ← 이런 줄이 오탐이 된다
 ...
 *
 * 상태 줄은 끝난 것과 안 끝난 것을 함께 담는 경우가 많다.
 *   TS-018  "원인 규명 완료: 수정은 하지 않음"
 *   ADR-011 "구현 완료 · 통합테스트 통과 / 실 PG 실증은 미실시"
 * 이런 줄은 참조 쪽의 "(보류)" "(미실시)"와 모순이 아니다. '완료'만 보면 오탐이 난다.
 *
 * 그래서 상태 줄에 미해결 표현이 하나라도 있으면 넘어간다. 일부 놓치는 경우가 생기지만,
 * 대상은 끝났는데 참조는 안 끝났다고 말하는 명백한 드리프트만 잡는다.
 */
