...
- ⚠️ **§4-②(Redis TLS)는 뒤집혔다.** 2026-08-25 확인: Terraform·매니페스트 모두 TLS **on**이다.
  §4-②와 §5 표의 해당 행은 **당시 결정의 기록**이며 현재 구성이 아니다.
- 날짜: 2026-08-05
...
> ⚠️ **셋 중 ②는 이후 닫혔다**(PR #174, Redis TLS on). 지금 열려 있는 것은 ①·③ 두 곳이다.
> ②를 지우지 않는 이유는 **무엇을 근거로 열었다가 왜 닫았는지**가 남아야 하기 때문이다.
...
**② ElastiCache 전송 암호화 — 끈다** ⚠️ **[뒤집힘 — 현재는 켜져 있다]**

> **PR #174에서 뒤집혔다.** 2026-08-25 기준 실제 구성:
>
> | 위치 | 값 |
> |---|---|
> | `modules/elasticache/variables.tf` `transit_encryption` | `default = true` |
> | `environments/demo/variables.tf` `redis_transit_encryption` | `default = true` |
> | `k8s/base/configmap.yaml` `REDIS_SSL_ENABLED` | `"true"` |
>
> 아래는 **2026-08-05 당시의 판단 기록**이다. 도달 경로가 없다는 논리 자체는 지금도 유효하고,
> 그럼에도 켠 것은 **앱 설정을 함께 바꿀 수 있게 됐기 때문**이다. 이 둘은 반드시 짝으로
> 움직인다 — 하나만 켜지면 파드가 영원히 Ready가 되지 않는다(`k8s/README.md`의 짝 표).

```
transit_encryption_enabled = false   # 당시 값. 현재는 true
```
...
| ~~**Redis TLS 활성화**~~ ⚠️ **뒤집힘** | 당시: 앱 설정 변경이 따라와야 하고, 프라이빗 서브넷 + SG 제한으로 도달 경로가 이미 없다. **→ PR #174에서 앱 설정과 함께 켰다. 현재 기본값은 `true`**(§4-②) |
...
- **되돌리기**: 열어 둔 곳은 전부 변수·설정 수준의 변경이다. 운영으로 전환한다면
  EKS API를 프라이빗으로 좁히고(배스천/VPN 도입), RDS 백업·`deletion_protection`을
  활성화한다. 그때는 이 ADR을 대체하는 새 문서를 남긴다.
  (**Redis TLS는 이 목록에서 빠졌다** — PR #174에서 이미 켰다. §4-② 참조.)
