...
public class AdminDlqService {
     ...
     *
     * 브로커 확인 전에 RETRIED로 바꾸지 않는다. send()는 비동기라, 확인 없이 마킹하면
     * 커밋 뒤에 온 브로커 실패를 아무도 보지 못해 DB는 "재처리됨"인데 메시지는 유실되고
     * 운영자 목록에서도 사라진다(TS-026). 그래서 OutboxRelay와 같이 .get()으로 확인한다.
     *
     * 발행이 실패하면 예외로 트랜잭션이 롤백돼 상태가 그대로 남고, 운영자가 다시 시도할 수 있다.
     * 반대로 발행 성공 후 커밋이 실패하면 중복 발행이 되는데, 컨슈머가 eventId 멱등을
