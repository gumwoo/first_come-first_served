...
public class DlqConsumer {
    ...
    /**
     * DLT는 불투명한 바이트로 받는다. 타입으로 받으면 안 된다(TS-020).
     *
     * DLT에는 두 종류가 들어온다: 리스너에서 실패한 정상 이벤트의 JSON과,
     * 애초에 역직렬화에 실패한 원본 바이트다. 후자를 OrderEvent로 받으려 하면
     * DLT 소비가 또 실패하는데 DLT에는 다시 보낼 곳이 없어 그 자리에서 무한 재시도가 된다.
     * 즉 독성 메시지가 원본 토픽에서 DLT로 이사할 뿐 아무것도 해결되지 않는다.
     *
     * 그래서 해석하지 않고 그대로 기록만 한다. 판단(재시도/폐기)은 사람이 admin API로 한다(ADR-008).
     */
