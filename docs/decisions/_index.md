...
| [ADR-015](ADR-015-queue-status-sync-polling-cost.md) | 대기열 상태 동기화 — onopen 재동기화로 복구를 만들고 폴링을 저빈도 안전망으로 전환. 폴링이 지금은 유일한 복구 경로라 먼저 걷어내면 정확성이 무너진다(주기는 측정 후 결정) | **Proposed** |
