 ...
 *
 * <p>SSE 알림은 트랜잭션 안에서 보내면 아직 커밋되지 않은 상태를 알린다. 뒤에서 예외가
 ...
 *
 * <p>왜 아웃박스가 아닌가. 반드시 도달해야 하는 이벤트(order.paid)는 아웃박스로 보낸다(ADR-010).
 * 좌석맵·주문상태 갱신 알림은 놓쳐도 폴링·재조회가 메우는 보조 수단이라(ADR-008, TS-012),
 * 가장 뜨거운 좌석 선점 경로에 DB 쓰기를 더하지 않는다.
 *
