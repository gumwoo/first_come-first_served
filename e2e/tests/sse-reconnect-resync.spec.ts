 ...
 * `onopen` 재조회가 <b>재연결 복구의 유일한 수단</b>이다. 그게 안 불리면 연결이 끊긴 사이
 * 다른 사람이 좌석을 가져가도 그 화면은 <b>영영 낡은 좌석맵을 보여준다.</b>
 * 사용자는 이미 팔린 좌석을 고르고 제출 단계에서야 거절당한다.
 *
 * <p>대기열 회귀 테스트(`queue-waiting.spec.ts`)와 같은 구조다 — SSE를 막아 이벤트를
 * 소실시키고, 풀었을 때 재연결만으로 복구되는지 본다. 초기 프레임 전송을 되돌리면 실패한다.
 */
test("SSE가 끊긴 사이 좌석이 팔려도 재연결하면 좌석맵이 갱신된다", async ({ page }) => {
  ...
  // 2) 다른 사람이 그 좌석을 가져간다. seat.held 이벤트는 SSE로만 나가고, 연결이 없어 소실된다.
  //    (이 요청은 page.request라 브라우저 라우트에 걸리지 않는다)
