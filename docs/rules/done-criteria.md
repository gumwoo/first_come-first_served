...
## 측정 (성능/동시성 개선이 포함된 슬라이스)
- [ ] 개선 전후를 동일 조건으로 측정해 `benchmarks/`에 before/after 커밋
- [ ] `docs/improvements/IMP-XXX.md` 작성 + `METRICS.md` 보드 갱신
- (naive→측정→개선→재측정 순서 권장. 끝나고 나면 측정 불가)
