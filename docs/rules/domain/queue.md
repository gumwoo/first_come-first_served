...
- ~~**발급 부분 실패 무롤백**: `SET NX` 성공 후 ZADD/HSET 등 후속이 실패하면 일시적 불일치.~~
  → **해소**: 예약(SET NX)과 등록(INCR·ZADD·HSET·EXPIRE·SADD)을 **한 Lua로 원자화**해 부분 상태가
  남지 않는다(ADR-002 §5-1). 죽은 토큰 회수 후 재발급도 옛 메타 정리까지 같은 원자 단위.
  단, **재사용 여부 판단은 애플리케이션 쪽 읽기**라 그 사이 상태가 바뀌면 회수 경로로 흡수된다.
