...
## 인증 (S01)
- `POST /auth/signup` — 가입(이름/이메일/비번/휴대폰인증/약관)
- `POST /auth/login` — 이메일+비번 → `{ accessToken, refreshToken }`
- `POST /auth/refresh` — refresh 토큰으로 재발급 (refresh는 Redis 저장)
- `POST /auth/logout`
- `GET /oauth2/authorization/{kakao|naver}` — 소셜 로그인 시작
- `GET /me` — 내 프로필
