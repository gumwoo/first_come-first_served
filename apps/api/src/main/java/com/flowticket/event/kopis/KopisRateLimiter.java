...
/**
 * KOPIS 호출 간격 제한기. KOPIS 이용 제한은 IP당 1초 10회이고, 넘기면 서비스가 중지된다.
 *
 * <p>단순히 느리게 하는 장치가 아니라 남의 서비스에 대한 약속이다. 초과하면 우리 키가
 * 막혀 시딩 자체를 못 하게 된다.
 *
 * <p>KOPIS로 나가는 모든 호출이 이 하나를 통과한다. 목록({@code fetchList})과
 * 상세({@code fetchDetail}) 둘 다 여기를 지나므로 합산이 설정값 이하로 유지된다. 둘이 별도
 * 제한기를 쓰면 합산이 2배가 되어 제한을 넘긴다.
 *
 * <p>왜 프로세스 안의 제어로 IP 총량이 지켜지나. 호출자가 동기화 배치 하나뿐이고, 그
