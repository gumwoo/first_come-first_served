 ...
 * 1인 구매 한도 계산 전용.
 *
 * 좌석 초과판매는 조건부 UPDATE가 막지만(ADR-003), 1인 한도는 단일 행에 표현되지 않는
...
public interface SeatQuotaRepository extends Repository<SeatHold, Long> {
     ...
     *
     * 단일 SQL이어야 한다. 두 문장으로 나누면 READ COMMITTED에서 결제 커밋 사이에 0매로 읽힌다.
    ...
    /**
     * (사용자, 공연) 단위 직렬화: pg_advisory_xact_lock, 트랜잭션 종료 시 자동 해제.
     *
     * 고정 행이 없어 행 락으로는 새 홀드 삽입(팬텀)을 못 막는다. 세션 락이 아니라 xact 변형이어야
