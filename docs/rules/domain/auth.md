...
- **동시 가입도 409다.** `existsByEmail` 사전 검사는 순차 요청만 막는다. 같은 이메일로 동시에 오면 둘 다
  "없음"을 보고 INSERT하고, 늦은 쪽이 `uq_users_email`에 걸린다. 이 위반은 500이 아니라 409로 돌려준다 —
  중복 가입은 서버 오류가 아니라 재시도하면 안 되는 충돌이다. [T]
  - 어느 제약(이메일/휴대폰)인지는 **다시 조회해서** 판별한다. 예외의 제약 이름 파싱은 드라이버·방언에 묶인다.
    진 쪽의 INSERT는 승자의 커밋까지 유니크 인덱스에서 대기하므로, 조회 시점에는 승자 행이 보인다.
  - 확인한 제약만 도메인 예외로 바꾸고, 그 외(NOT NULL·FK 등)는 원 예외를 그대로 올린다(주문 생성과 같은 규칙, TS-014).
  - 가입 메서드는 `NOT_SUPPORTED`여야 한다. 클래스 레벨 `@Transactional(readOnly = true)`에 참여하면 INSERT가
    read-only로 실패하고, 캐치가 트랜잭션 안에 있어 rollback-only에 걸린다(TS-014와 같은 함정).
