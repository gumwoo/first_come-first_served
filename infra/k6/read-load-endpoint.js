...
//
// 혼합 부하로는 엔드포인트별 비용을 가를 수 없다. 캐시로 얻을 수 있는 개선의 상한은 캐시 가능한
// 경로가 전체 비용에서 차지하는 비율이라(Amdahl), 엔드포인트를 하나씩 같은 도착률로 잰다.
// 요청 비율과 비용 비율은 다르다(seats가 요청의 50%라도 비용의 50%라는 보장이 없다).
//
// 실행(엔드포인트별로 각각):
...
//   k6 run ... -e EP=detail ...
//
// 포화 아래에서 재야 한다. 무릎(600~650) 위에서는 큐잉 때문에 지연이 부풀어
