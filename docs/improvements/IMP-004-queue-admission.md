...
| over-admit | 0 | **관측된 모든 샘플에서 ≤ 100** — 3회 독립 실행, 총 1,117샘플의 최대값이 100 |

즉 **"Lua 원자화가 다중 파드에서도 정원 불변식을 유지한다"** 가 실물로 확인됐다.
상승 구간의 중간값(`admit=37`)도 1건 포착돼, 정원까지 차오른 뒤 그 위로 넘지 않는 모양이 관측됐다.
...
연속 구간이어야 한다. 실제로 두 실행 모두 그랬다 — run-2는 **남은 2,800개가 201~3000**,
run-3은 **남은 2,900개가 101~3000**으로 **둘 다 구멍 0**이다.
종료 스냅샷에서 늦게 도착한 토큰이 먼저 빠져나간 흔적이 0건이다.

⚠️ **한계 — "over-admit 0을 증명했다"고 쓰지 않는다.** 이 측정이 보인 것은
**관측된 모든 시점에서 `admitcount` ≤ 100**이다. 100~200ms 샘플 사이에 순간적으로
`101 → 100`이 됐는지는 이 방식으로 배제할 수 없다. 순서 보존도 마찬가지로
**종료 스냅샷에서 추월 흔적 0**이며, 승격을 event-by-event로 캡처한 것은 아니다.
