 ...
 *
 * <p>E2E는 이 값을 600000(10분)으로 준다. 그 값은 운영 후보가 아니다.
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // 최종 안전망. setInterval이 아니라 매번 다시 예약하는 이유는 주기가 변하기 때문이다.
        //
        // SSE가 성립해 있는 동안에는 주기를 배로 늘린다(상한 POLL_MAX_MS). 재연결 공백은
        // onopen 재조회가 닫으므로, 그 구간의 복구를 폴링에 의존하지 않아도 된다.
        // SSE가 한 번도 열리지 않았거나 끊긴 동안에는 최소 주기를 유지한다. 그때는 폴링이
        // 유일한 복구 경로다.
        //
        // 대가: SSE가 열려 있는데도 이벤트가 유실되는 경우의 복구가 최대 POLL_MAX_MS까지
        ...
        // 여기서 하는 일은 폴링을 다시 촘촘하게 되돌리는 것뿐이다. 연결이 끊긴 구간이
