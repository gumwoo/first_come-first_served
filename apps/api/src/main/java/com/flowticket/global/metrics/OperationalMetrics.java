 ...
 *
 * <p>왜 필요한가: 이 값들은 사용자 요청은 전부 성공하는데 뒤에서만 쌓이는 종류다.
 * 에러율·지연에 나타나지 않으므로, 세어서 내보내지 않으면 알아챌 방법이 없다(TS-032·ADR-008·ADR-016).
 *
 * <p>관리자 콘솔의 임계치({@code AlertSettings})와는 역할이 다르다. 그쪽은 볼 때
 * 알려주고, 이쪽은 안 봐도 알려주기 위한 입력이다. 임계치를 공유하지 않는다:
 * 콘솔 값은 운영 중 바꿀 수 있고 Prometheus 규칙은 Git에 고정돼, 하나로 묶으면 어느 쪽이
 * 진실인지 모호해진다.
 *
 * <p>게이지는 스크랩 시점에 평가된다. 스크랩 주기(15초, servicemonitor-api.yaml)마다
 * COUNT 질의가 한 번씩 돈다.
...
public class OperationalMetrics {
     ...
     *
     * <p>DB가 흔들릴 때 지표 수집이 스크랩 엔드포인트 전체를 깨뜨리면 안 된다.
