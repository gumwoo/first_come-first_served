...
```
파괴적 DDL 발견 → CI 실패 → PR 본문에 다음을 적고 승인
  · 어떤 릴리스에서 Expand가 선행되었는가
  · 구버전 Pod가 이 변경으로 깨지지 않는 근거
```

→ 하네스 정적 가드로 승격할 후보다(현재는 사람이 지킨다).
...
- **자동 테스트로 지켜지지 않는다.** Expand-Contract 규칙도, SSE 복원력도 현재는 사람이 지킨다.
  전자는 하네스 가드로 승격 가능하고, 후자는 롤링 실측이 유일한 검증 수단이다.
