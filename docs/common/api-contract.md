...
- `GET /events?page=&size=&genre=&status=&from=&to=` — 목록(페이징/필터). 응답 `{ items, page, size, total }`
- `GET /events/:id` — 상세(공연정보 + 좌석등급/가격/잔여재고)
- `GET /events/popular` — 인기 TOP / 실시간 랭킹(현재 ON_SALE 최신 8개 임시, 조회수 집계로 교체 예정)
- `GET /search?q=&genre=&status=&page=&size=` — 키워드 + 장르/상태 필터 검색(페이징)
