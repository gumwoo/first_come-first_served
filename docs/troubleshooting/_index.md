...
| [TS-037](TS-037-rds-redis-failover-app-behavior.md) | ADR-012가 A(실증)로 약속한 RDS·Redis Multi-AZ 페일오버를 실제로 눌렀다 — 둘 다 파드 재시작 없이 자동 복구했고 관찰한 토큰 1개의 `ADMITTED` 상태도 유실 0이었으나, **RDS는 Hikari connection-timeout이 설정돼 있지 않아 기본 30초를 다 쓴 뒤 500**(최소 두 파드에서 커넥션 획득 타임아웃 관측 — 누적 57·40), Redis는 p95가 19.5초까지 상승 | S09 | 장애 주입 실증 | **실증 완료**(RDS 93/8,817 · Redis 263/5,287) + 결함 후보 1건. 개선 재측정은 별도 IMP로 분리 — 설정을 바꾸면 기준선이 달라진다 |
| [TS-038](TS-038-teardown-untracked-vpc-leftovers.md) | 철거에서 `terraform destroy`가 서브넷 `DependencyViolation`으로 실패했다 — 막고 있던 둘(**VPC CNI 고아 ENI**, **EKS가 만든 클러스터 SG**)이 **terraform 밖에서 만들어진 것**이라 destroy 대상이 아니었다. 게다가 래퍼 종료 코드 때문에 **실패가 성공으로 보였다** | S09 | 철거 사건 회고 | **해결 · `tear-down.sh` 6/7단계로 반영**. ⚠️ 실패 경로 자체는 재현 검증 못 함 |
| [TS-039](TS-039-localdatetime-offset-seat-hold-expired.md) | 좌석을 고르자마자 "만료" — 응답의 `LocalDateTime`에 오프셋이 없어 브라우저가 UTC 서버 시각을 KST 로컬로 읽어 9시간 과거로 판정했다. 로컬(JVM·브라우저 모두 KST)에서는 재현되지 않는다 | S04 | 환경 의존 결함(직렬화) | **해결**(서버 존 오프셋 직렬화 + 하네스 backend 16 · k8s 7) |
| [TS-040](TS-040-toss-cancel-amount-missing.md) | 수수료를 뗀 환불이 전액 취소로 나갈 수 있었다 — `RefundPolicy`가 계산한 취소 금액을 Toss 어댑터가 본문에 싣지 않아, `cancelAmount` 없는 요청이 전액 취소로 처리된다. Mock 게이트웨이가 금액을 보지 않아 기존 환불 테스트로는 걸리지 않았다 | S06(환불) | 어댑터 결함(장부 불일치) | **해결**(취소 금액 전달 + 어댑터 요청 본문 테스트 4건. 되돌려서 2건이 실패하는 것을 확인) |
