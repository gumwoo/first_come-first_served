...
| [ADR-019](ADR-019-self-proxy-to-collaborator.md) | 자기 프록시 주입(`ObjectProvider<Self>`) 대신 협력자로 나눈다 — 프레임워크의 프록시 구현이 서비스 의존성에 드러나고 단위 테스트가 자기 자신을 배선해야 했다. 경계의 성격에 따라 쓰기 협력자·진입점 분리·조회/명령 분리로 나눈다. **락·트랜잭션의 위치는 옮기지 않는다** | Accepted(Auth·KOPIS 적용, 나머지 후속) |
