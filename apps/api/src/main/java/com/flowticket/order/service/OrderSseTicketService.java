 ...
 *
 * <p>그래서 구독 전용 단명 티켓을 URL로 싣는다. 대기열이 이미 같은 모양이다 —
 * {@code /sse/queue/{token}}은 추측 불가능한 값 자체가 자격증명이다(ADR-002).
 *
 * <p>URL에 실리는 자격증명이라는 점은 그대로 비용이다. 접근 로그·리퍼러에 남는다.
 * 그래서 (1) access token이 아니라 이 용도로만 쓰이는 타입이고, (2) 주문 하나에
 * 묶여 있고, (3) TTL이 짧다. 세 가지가 함께 있어야 노출의 값이 낮아진다.
 *
 * <p>TTL이 뜻하는 것을 정확히 읽어야 한다. {@link #verify}는 구독을 맺을 때만
 * 돌고, 티켓이 만료돼도 이미 성립한 스트림은 끊기지 않는다. 즉 {@code jwt.sse-ticket-ttl}은
 * 새 구독을 시작할 수 있는 창을 줄이는 값이지 노출 상한이 아니다.
 *
 * <pre>
 ...
 * 더 짧지만, 그것은 인프라의 부작용이지 이 설계가 보장하는 성질이 아니다 —
 ...
 * 클레임에 남겨 두는 것은 누구에게 발급된 티켓인지 추적하기 위해서다.
 */
@Service
public class OrderSseTicketService {
     ...
     *
     * <p>401이어야 하는 이유가 있다. {@code EventSource}는 2xx가 아닌 응답을 받으면
