...
public interface SeatQuotaRepository extends Repository<SeatHold, Long> {
    ...
    /**
     * 이 사용자가 이 공연에서 실제로 붙들고 있는 서로 다른 좌석 수.
     *
     * <p>점유의 진실원은 두 곳뿐이다:
     * <pre>
     ...
     *
     * <p>반드시 단일 SQL이어야 한다. "PAID 좌석 수"와 "HELD 좌석 수"를 두 문장으로
     * 나누어 읽으면 READ COMMITTED에서 문장마다 스냅샷이 달라, 그 사이에 결제가 커밋되면
     * 앞 문장은 결제 전(PAID 0)을, 뒤 문장은 결제 후(HELD 0)를 보아 0매로 계산된다.
     * 한 문장이면 결제 전(0+4) 또는 결제 후(4+0) 중 하나만 보므로 항상 4다.
     *
     * <p>UNION은 중복을 제거한다. 주문 생성 후에는 같은 좌석이 order_items와
     ...
     *
     * <p>왜 행 락이 아닌가: 현재 데이터 모델에 (사용자, 공연) 한도를 대표하는 고정 행이
     * 없다. 기존 홀드 행을 잠가도 새 홀드의 삽입은 막지 못한다(팬텀) —
     ...
     * 않은 이유는 비정규화 카운터를 해제·만료·환불 경로마다 정확히 줄여야 하는데,
     * 이 프로젝트에서 반복해서 깨진 곳이 바로 그 경로들이기 때문이다(TS-007·010·011).
     * 파생 상태를 늘리지 않고 원본에서 세는 쪽을 골랐다.
     *
     * <p>왜 Redis 분산락이 아닌가: 리스 만료와 트랜잭션 수명이 따로 놀아 만료 후 소유자가 둘이
     * 될 수 있다. advisory lock은 데이터와 같은 시스템에서 잡히고 커밋·롤백과 함께 풀린다.
     *
