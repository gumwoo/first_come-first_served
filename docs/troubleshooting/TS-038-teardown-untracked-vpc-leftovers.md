...
## 6. 회귀 하나를 스스로 만들었다 — CR이 LF가 됐다

리뷰가 3단계에서 이걸 잡았다.

```
kubectl get pv ... | tr -d '<LF>' | grep -E '^vol-' > "$OWNED_VOLS"
```

원래는 `tr -d '<CR>'`(Windows CR 제거)인데 **줄바꿈을 지우게 돼 있었다.** 그러면 볼륨 ID가
전부 한 줄로 붙는다.

```
vol-aaa          vol-aaavol-bbbvol-ccc
vol-bbb    →     (존재하지 않는 ID 하나)
vol-ccc
```

4단계가 `while read -r v`로 하나씩 조회하므로, **볼륨이 여러 개면 이 클러스터 PV가 가리킨
볼륨을 하나도 못 지운다.** 3단계가 지키려던 *"소유를 증명한 것만 지운다"*가 그대로 무너진다.

### 원인 — 파일을 읽는 방식

```python
io.open(p, encoding='utf-8').read()      # universal newline: 단독 CR → LF
io.open(p, 'w', newline='').write(s)     # 쓸 때는 그대로 보존
```

**읽으면서 CR이 LF로 바뀌고, 쓸 때 그 LF가 그대로 저장된다.** 파일 안의 raw CR 바이트가
전부 오염된다. 스크립트를 편집할 때마다 조용히 일어나고, diff에서도 공백처럼 보여 눈에 안 띈다.

### 확인한 범위

`git ls-files` 전체를 바이트로 훑어 같은 손상을 찾았다.

```
scripts/tear-down.sh    3곳   ← 전부 이번 변경에서 생김
그 외                   없음
```

다른 스크립트가 무사한 이유는 raw CR 바이트가 아니라 **역슬래시+r 두 글자 표기**를 쓰기 때문이다.
main도 깨끗하다. 3단계는 복구 후 **main과 바이트 동일**함을 확인했다.
