...
public class JwtProvider {
    ...
    /**
     * 특정 주문 구독에만 유효한 티켓. 대상 주문을 클레임에 박아, 티켓이 새더라도 그 주문
     * 하나만 열린다. TTL은 짧게 둔다({@code jwt.sse-ticket-ttl}, 기본 300초) — URL에 실리는
     * 자격증명이라 노출 창을 줄이는 것이 목적이고, 만료 후에는 프론트가 새로 발급받는다.
     *
     * <p>TTL은 "새 연결을 시작할 수 있는 기간"이지, 이미 성립한 스트림의 수명이 아니다.
