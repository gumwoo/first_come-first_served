...
const cases = [
  ...
  { name: "be-unvalidated-paging", script: "backend/check.mjs", expect: "페이징 파라미터를 검증 없이 받는다:", env: { HARNESS_API_DIR: "harness/fixtures/violations/be-unvalidated-paging" } },
  ...
  { name: "be-gradle-version-drift", script: "backend/check.mjs", expect: "Gradle 버전이 어긋난다:", env: { HARNESS_API_DIR: "harness/fixtures/violations/be-gradle-version-drift" } },
  { name: "be-gradle-version-unreadable", script: "backend/check.mjs", expect: "Gradle 버전을 읽지 못했다:", env: { HARNESS_API_DIR: "harness/fixtures/violations/be-gradle-version-unreadable" } },
