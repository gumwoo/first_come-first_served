# ADR-010 · 트랜잭셔널 아웃박스 — DB↔Kafka 정합성(at-least-once 발행 + 소비자 멱등), 정산은 분리

> ⚠️ 이 문서의 제목은 원래 **"exactly-once 발행"**이었다. 구현은 그때도 지금도
> **at-least-once**다(§결정 3: publish-then-mark라 크래시 시 재발행된다). 즉 제목이
> **자기 본문과 모순**이었고, 구현보다 세게 주장하고 있었다. 2026-08-25에 바로잡았다.
>
> 중복이 사용자에게 보이지 않는 이유는 발행이 정확해서가 아니라 **소비자가 멱등**하기
> 때문이다(§결정 4: `eventId` + Redis SETNX). 둘은 다른 보장이다.
