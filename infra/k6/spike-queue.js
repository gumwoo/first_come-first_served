// 시나리오 3) 스파이크 — 오픈 순간 대량 동시 진입.
//
// 티켓 오픈은 정의상 스파이크다. 평상시 0에서 순간 수천으로 튀는 것이 이 시스템이 존재하는
// 이유이고, 대기열은 그 순간을 위해 만든 장치다.
//
// 증명할 명제 셋:
//   1) over-admit 0     — 도착량과 무관하게 동시 ADMITTED가 정원(100)을 넘지 않는다
//   2) 순서 보존         — 진입 순번(rank) 순서대로 승격된다. 못 지키면 "선착순"이 거짓이 된다
//   3) 나머지는 대기      — 입장 못 한 사람이 에러가 아니라 WAITING 상태를 받는다
//
// 3)이 특히 중요하다. 9,900명이 500을 받으면 "막았다"가 아니라 "터졌다"이다.
//
...
// open()은 init 컨텍스트에서 VU마다 실행된다. 3,000 VU가 각각 수 MB JSON을 파싱하면
// k6 프로세스가 메모리로 죽는다("fatal error: runtime: cannot allocate memory").
// SharedArray는 파싱 결과를 VU 전체가 공유해 그 문제를 없앤다.
...
export default function () {
  ...
  // 명제 2)(순서 보존) 검증용. 진입 시점의 rank를 사용자별로 남긴다 — k6는 VU별 데이터를
  ...
  check(res, {
    ...
    // 정원 초과분은 에러가 아니라 WAITING 이어야 한다(명제 3)).
